...
typedef struct nk_gpu_dev_video_mode {
    ...
    uint32_t width;                  // width and height of screen (pixels or characters)
    uint32_t height;

    // for graphics, channel offsets into a pixel (-1 means channel does not exist)
    // for text, channel 0 is the character, channel 1 is the attribute
    uint8_t  channel_offset[4];
#define NK_GPU_DEV_CHANNEL_OFFSET_RED   0
...
#define NK_GPU_DEV_TEXT_CURSOR_BLINK 2
    ...
    // call with NULL to remove box or region
    ...
    // change the mouse cursor image to bitmap
    // must be 64 in width and height
...
// switch to a specific mode, ideally one returned from get_available_modes
...
// initializes pixels
// Arguments
//	m: nk_gpu_dev_video_mode_t* - the current graphics mode
//	p: nk_gpu_dev_pixel_t* - the pixel to set RGBA components for
//	r: uint8_t - 8-bit RED component value
//	g: uint8_t - 8-bit GREEN component value
//	b: uint8_t - 8-bit BLUE component value
//	a: uint8_t - 8-bit ALPHA component value
