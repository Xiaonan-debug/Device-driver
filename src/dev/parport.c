...
struct parport_state {
    ...
    enum {READY=0, BUSY} driver_status; // tracks the status of the driver
};
...
// wait for the attached device (e.g., printer) to be ready
// should not return until the device is ready
static void wait_for_attached_device(struct parport_state *s)
{
    stat_reg_t stat;
    
    // here you will check the device's status to see if it is busy
    //	1. Read the status register for the device
    //  2. Check if the device is busy (note: that bit is active low, so zero means busy)
    //	3. If the device is busy, wait for a short time, and then return to step 1
    //
    // WRITE ME!
    while (true) {
...
static int read_write(void *state, uint8_t *data, int write)
{
    ...
    if (s->driver_status!=READY) {
	DEBUG("not ready\n");
	rc = -1; // error
	goto out;
    }

    // we also check that the device itself is ready
    ...
    if (write) {
        // here you will:
	    //
	    // 1. mark the driver as busy
	    // 2. set directionality to output using the control register
	    // 3. write the data to the data register
	    // 4. strobe the attached device via the control register
	    //
	    // WRITE ME!

        DEBUG("Set the driver as busy\n");

        // 1. mark the driver as busy
        s->driver_status = BUSY;
...
static int interrupt_handler(excp_entry_t * excp, excp_vec_t vec, void *state)
{
    ...
    //    interrupted, we need to explicitly renable interrupts.
    ...
    // The interrupt has occurred, so the device has completed the action

    // Here you will:
    //	1. mark that the driver is no longer busy
    //	2. signal anyone waiting on the device
    //
    // WRITE ME!

    int tryLockResult = STATE_TRY_LOCK(s);
    if (tryLockResult == 0) {
        // 1. mark that the driver is no longer busy
        s->driver_status = READY;

        // 2. signal anyone waiting on the device
        nk_dev_signal(&(s->dev->dev));
        STATE_UNLOCK(s);
...
static int init(struct parport_state *s)
{
    ...
    // read in the initial ctrl state for debugging
    ctrl.val = inb(CTRL_PORT(s));
