...
static int set_mode(void *state, nk_gpu_dev_video_mode_t *mode)
{
    ...
    if (transact_rw(d->virtio_dev,
        ...
        ERROR("failed to create screen resource (transaction failed)\n");
        return -1;
    }

    CHECK_RESP(create_2d_resp, VIRTIO_GPU_RESP_OK_NODATA, "failed to create screen resource\n");

    DEBUG("create screen resource\n");
    ...
    memset(d->frame_buffer, 0x77, fb_length);
    ...
    backing_entry.addr = (uint64_t)d->frame_buffer;
    backing_entry.length = fb_length;

    if (transact_rrw(d->virtio_dev,
                    0,
                    &backing_req,
                    sizeof(backing_req),
                    &backing_entry,
                    ...
                    &backing_resp,
                    sizeof(backing_resp))) {
        ERROR("failed to associate with framebuffer (transaction failed)\n");
        return -1;
    }

    CHECK_RESP(backing_resp, VIRTIO_GPU_RESP_OK_NODATA, "failed to associate with framebuffer\n");

    DEBUG("associate with framebuffer\n");
    ...
    setso_req.r.x = 0;
    setso_req.r.y = 0;
    setso_req.r.width = pm->r.width;
    setso_req.r.height = pm->r.height;
    ...
    if (transact_rw(d->virtio_dev,
                    ...
                    &setso_resp,
                    sizeof(setso_resp))) {
        ERROR("failed to associate framebuffer (transaction failed)\n");
        return -1;
    }

    CHECK_RESP(setso_resp, VIRTIO_GPU_RESP_OK_NODATA, "failed to associate with scanout\n");

    DEBUG("associate with scanout\n");
...
static int flush(void *state)
{
    ...
    //
    // WRITE ME
    // transfer from framebuffer to resource
    xfer_req.hdr.type = VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D;
    xfer_req.resource_id = SCREEN_RID;
    xfer_req.r.x = 0;
    xfer_req.r.y = 0;
    xfer_req.r.width = pm->r.width;
    xfer_req.r.height = pm->r.height;
    ...
    if (transact_rw(d->virtio_dev,
                    ...
                    &xfer_resp,
                    sizeof(xfer_resp))) {
        ERROR("failed to transfer from framebuffer to resource (transaction failed)\n");
        return -1;
    }

    CHECK_RESP(xfer_resp, VIRTIO_GPU_RESP_OK_NODATA, "failed to transfer from framebuffer to resource\n");

    DEBUG("transfer from framebuffer to resource\n");
    ...
    flush_req.r.x = 0;
    flush_req.r.y = 0;
    flush_req.r.width = pm->r.width;
    flush_req.r.height = pm->r.height;
    ...
    if (transact_rw(d->virtio_dev,
                    ...
                    &flush_resp,
                    sizeof(flush_resp))) {
        ERROR("failed to copy from the resource to the screen (transaction failed)\n");
        return -1;
    }

    CHECK_RESP(flush_resp, VIRTIO_GPU_RESP_OK_NODATA, "failed to copy from the resource to the screen\n");

    DEBUG("copy from the resource to the screen\n");
...
static int graphics_set_clipping_box(void *state, nk_gpu_dev_box_t *box)
{
    ...
    // WRITE ME
    if (box != NULL) {
        d->clipping_box.x = box->x;
        d->clipping_box.y = box->y;
        d->clipping_box.width = box->width;
        d->clipping_box.height = box->height;
    } else {
        d->clipping_box.x = d->frame_box.x;
        d->clipping_box.y = d->frame_box.y;
        d->clipping_box.width = d->frame_box.width;
        d->clipping_box.height = d->frame_box.height;
    }
    return 0;
}
...
static inline uint32_t get_channel(uint32_t n0, uint32_t n1, nk_gpu_dev_bit_blit_op_t op, int offset) {
    uint32_t n = 0;
    n0 = (n0 >> offset) & 0xff;
    n1 = (n1 >> offset) & 0xff;
    switch (op) {
    case NK_GPU_DEV_BIT_BLIT_OP_PLUS:
        n = n0 + n1 > 0xff ? 0xff : n0 + n1;
        break;
    ...
    case NK_GPU_DEV_BIT_BLIT_OP_DIVIDE:
        ...
        break;
    default:
        ERROR("Unexpected bit blit operation: %d\n", op);
        return n;
    }
    n = n << offset;
    return n;
...
static inline uint32_t get_raw(uint32_t n0, uint32_t n1, nk_gpu_dev_bit_blit_op_t op) {
    return get_channel(n0, n1, op, 0) |
        get_channel(n0, n1, op, 8) |
        get_channel(n0, n1, op, 16) |
        get_channel(n0, n1, op, 24);
}
...
static void apply_with_blit(nk_gpu_dev_pixel_t *oldpixel, nk_gpu_dev_pixel_t *newpixel, nk_gpu_dev_bit_blit_op_t op)
{
    ...
    // WRITE ME - other cases
        ...
        case NK_GPU_DEV_BIT_BLIT_OP_XNOR:
            ...
            break;
        case NK_GPU_DEV_BIT_BLIT_OP_PLUS:
            oldpixel->raw = get_raw(oldpixel->raw, newpixel->raw, NK_GPU_DEV_BIT_BLIT_OP_PLUS);
            break;
        case NK_GPU_DEV_BIT_BLIT_OP_MINUS:
            oldpixel->raw = get_raw(oldpixel->raw, newpixel->raw, NK_GPU_DEV_BIT_BLIT_OP_MINUS);
            break;
        case NK_GPU_DEV_BIT_BLIT_OP_MULTIPLY:
            oldpixel->raw = get_raw(oldpixel->raw, newpixel->raw, NK_GPU_DEV_BIT_BLIT_OP_MULTIPLY);
            break;
        case NK_GPU_DEV_BIT_BLIT_OP_DIVIDE:
            oldpixel->raw = get_raw(oldpixel->raw, newpixel->raw, NK_GPU_DEV_BIT_BLIT_OP_DIVIDE);
            break;
        default:
	        oldpixel->raw = newpixel->raw;
	    break;
...
static inline int graphics_draw_pixel(void *state, nk_gpu_dev_coordinate_t *location, nk_gpu_dev_pixel_t *pixel)
{
    ...
    if (in_box(&d->frame_box, location) &&
        in_box(&d->clipping_box, location)) {
        apply_with_blit(
            get_pixel_pointer(d, location->x, location->y),pixel,
            NK_GPU_DEV_BIT_BLIT_OP_COPY);
    }
    return 0;
}

static int BresenhamLow(nk_gpu_dev_coordinate_t *start, nk_gpu_dev_coordinate_t *end, struct virtio_gpu_dev *d, nk_gpu_dev_pixel_t *pixel) {
    ...
    for (uint32_t x = start->x; x <= end->x; x++) {
        graphics_draw_pixel(d, &(nk_gpu_dev_coordinate_t){x, y}, pixel);
        if (D > 0) {
            y = y + yi;
            D = D + 2 * (dy - dx);
        } else {
...
static int BresenhamHigh(nk_gpu_dev_coordinate_t *start, nk_gpu_dev_coordinate_t *end, struct virtio_gpu_dev *d, nk_gpu_dev_pixel_t *pixel) {
    int dx = end->x - start->x;
    int dy = end->y - start->y;
    int xi = 1;
    if (dx < 0) {
...
static int Bresenham(nk_gpu_dev_coordinate_t *start, nk_gpu_dev_coordinate_t *end, struct virtio_gpu_dev *d, nk_gpu_dev_pixel_t *pixel) {
    int x0 = start->x;
    int y0 = start->y;
    int x1 = end->x;
    int y1 = end->y;
    ...
    if (dy <= dx) {
        return (x0 <= x1) ? BresenhamLow(start, end, d, pixel) : BresenhamLow(end, start, d, pixel);
    } else {
        return (y0 <= y1) ? BresenhamHigh(start, end, d, pixel) : BresenhamHigh(end, start, d, pixel);
    }
}
...
static int graphics_draw_poly(void *state, nk_gpu_dev_coordinate_t *coord_list, uint32_t count, nk_gpu_dev_pixel_t *pixel)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        graphics_draw_line(d, &coord_list[i], &coord_list[(i + 1)%count], pixel);
    }
    return 0;
}
...
static int graphics_fill_box_with_pixel(void *state, nk_gpu_dev_box_t *box, nk_gpu_dev_pixel_t *pixel, nk_gpu_dev_bit_blit_op_t op)
{
    ...
    //
    // WRITE ME
    for (uint32_t x  = box->x; x < box->x + box->width; x++) {
        for (uint32_t y = box->y; y < box->y + box->height; y++) {
            if (in_box(&d->frame_box, &(nk_gpu_dev_coordinate_t){x, y}) &&
                in_box(&d->clipping_box, &(nk_gpu_dev_coordinate_t){x, y})) {
                apply_with_blit(get_pixel_pointer(d, x, y),pixel,op);
            }
        }
    }
    return 0;
...
static int graphics_fill_box_with_bitmap(void *state, nk_gpu_dev_box_t *box, nk_gpu_dev_bitmap_t *bitmap, nk_gpu_dev_bit_blit_op_t op)
{
    ...
    for (uint32_t x  = box->x; x < box->x + box->width; x++) {
        for (uint32_t y = box->y; y < box->y + box->height; y++) {
            if (in_box(&d->frame_box, &(nk_gpu_dev_coordinate_t){x, y}) &&
                in_box(&d->clipping_box, &(nk_gpu_dev_coordinate_t){x, y})) {
                uint32_t bitmap_x = (x - box->x) % bitmap->width;
                uint32_t bitmap_y = (y - box->y) % bitmap->height;
                nk_gpu_dev_pixel_t* pixel = get_bitmap_pixel_pointer(bitmap, bitmap_x, bitmap_y);
                apply_with_blit(get_pixel_pointer(d, x, y),pixel, op);
            }
        }
    }
...
static nk_gpu_dev_box_t *source_region(struct virtio_gpu_dev *d, nk_gpu_dev_box_t *source_box)
{
    nk_gpu_dev_box_t *source_region = malloc(sizeof(nk_gpu_dev_box_t));
    uint32_t sr_width = 0;
    uint32_t sr_height = 0;
    uint32_t sr_x = 0;
    uint32_t sr_y = 0;
    int foundOrigin = 0;
    for (uint32_t x  = source_box->x; x < source_box->x + source_box->width; x++) {
        for (uint32_t y = source_box->y; y < source_box->y + source_box->height; y++) {
            if (in_box(&d->frame_box, &(nk_gpu_dev_coordinate_t){x, y}) &&
                in_box(&d->clipping_box, &(nk_gpu_dev_coordinate_t){x, y})) {
                if (!foundOrigin) {
                    sr_x = x;
    ...
    source_region->x = sr_x;
    source_region->y = sr_y;
    source_region->width = sr_width;
    source_region->height = sr_height;
...
//    DEBUG("FRAMEBOX: (%u,%u) (%u,%u)\n", d->frame_box.x, d->frame_box.y, d->frame_box.width, d->frame_box.height);
//    DEBUG("CLIPBOX: (%u,%u) (%u,%u)\n", d->clipping_box.x, d->clipping_box.y, d->clipping_box.width, d->clipping_box.height);
//    DEBUG("SOURCEBOX: (%u,%u) (%u,%u)\n", source_box->x, source_box->y, source_box->width, source_box->height);
    nk_gpu_dev_box_t *sr = source_region(d, source_box);
//    DEBUG("SOURCEREGION: (%u,%u) (%u,%u)\n", sr->x, sr->y, sr->width, sr->height);

    if (sr->width == 0 || sr->height == 0) {
        return 0;
    }
    for (uint32_t x  = dest_box->x; x < dest_box->x + dest_box->width; x++) {
        for (uint32_t y = dest_box->y; y < dest_box->y + dest_box->height; y++) {
            if (in_box(&d->frame_box, &(nk_gpu_dev_coordinate_t){x, y}) &&
                in_box(&d->clipping_box, &(nk_gpu_dev_coordinate_t){x, y})) {
                uint32_t source_x = (x - dest_box->x) % sr->width + sr->x;
                uint32_t source_y = (y - dest_box->y) % sr->height + sr->y;
                apply_with_blit(get_pixel_pointer(d, x, y),get_pixel_pointer(d, source_x, source_y), op);
            }
        }
    }
    flush(d);
